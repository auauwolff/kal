...
- ✅ **Gem rule rework + Save-as-meal polish** (2026-04-27): gems are now awarded server-side on the **first entry of the day per category** (Breakfast / Lunch / Dinner / Snack / Exercise) — 5 gems per category, max 25/day. Subsequent entries award nothing, closing the water-spam farm path called out in §2.2. Exercise logs now award gems too (previously didn't). Payouts animate as five staggered +1 bursts (180 ms apart) for a Duolingo-style cascade; reduced-motion users still get a single combined burst. `users.backfillMyGems` migrated to count distinct `(date, mealType)` pairs + distinct exercise dates without shrinking existing balances. Diary UI: section-level **Save as meal** button (`BookmarkAddOutlined`) now lives on the **left** of each meal header (opposite the `+`), disabled when the section is empty. `AddFoodDialog` Foods/Meals tabs switched to the orange primary palette.
- ⏭️ **Next:** Wire weight logs + Stats queries, copy-from-any-day, barcode fallback via Open Food Facts, weekly-adjustment cron, and CSV export.
...
- **Top app bar:** **gem balance chip** on the left (`Diamond` icon + count, sapphire `info.main` when active, dimmed `text.disabled` at 0; wired to `user.gemBalance` — earn rules in §8). Phase 2 placeholder rule (shipped 2026-04-27): the **first entry of the day per category** (Breakfast / Lunch / Dinner / Snack / Exercise) awards 5 gems server-side; subsequent entries in the same category award nothing. Cap is 25 gems/day from logging — consistent with §2.2 and resilient to spam. The 5 gems pay out as five staggered +1 animations (180 ms apart): per pulse a Web Audio level-up jingle, `web-haptics` "buzz" haptic, and a canvas particle burst from screen-center homing into the chip (see `src/components/ParticlesProvider.tsx`). Reduced-motion users get a single combined burst. Right side of the bar: dark-mode toggle (the only place it lives — light mode is the default and Settings does not duplicate the toggle) and avatar menu (profile/settings/sign-out). The streak chip moved to the Diary day-header row — streaks count logging days, so they belong with the date. MUI `AppBar` with sticky positioning.
...
- **Earn rules** (subject to tuning):
  - +5 per **first entry of the day per category** — Breakfast / Lunch / Dinner / Snack / Exercise. Cap 25/day from logging. (Already shipped as the Phase 2 placeholder on 2026-04-27; carry forward into Phase 3.)
...
1. **Finish the remaining real-data flows.** Diary meal entries, profile/targets, exercise logs, and Stats now use Convex queries/mutations. Next: add weight quick-log / edit flows, richer stats queries, and keep Diary targets pointed directly at Convex.
